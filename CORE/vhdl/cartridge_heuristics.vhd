...
-- cart_game_n = 0
--
-- Detect falling edge of (cart_io1) and then check if ram_we=1 and ram_addr=x"DE00" (i.e. writing to DE00)
-- AND less than 200 clk_main_i cycles later (otherwise, if it comes later than 200 cycles, then it is not an EF3)
-- Detect falling edge of (cart_io1) and then check if ram_we=1 and ram_addr=x"DE0E" (i.e. writing to DE0E)
...
--
-- The addresses above are C64 KERNAL addresses, so this only ever triggers while the
-- C128 runs in C64 mode. That is exactly the case in which an EF3 is used, because the
-- EF3 pulls EXROM and GAME low and therefore forces the C128 into C64 mode.
--
...
architecture synthesis of cartridge_heuristics is
   ...
   -- This is a generic down-counter to handle timeouts while waiting for some event.
      ...
      if rising_edge(clk_main_i) then
         ...
         case state_ef3 is
            when IDLE_ST =>
               ...
               if cart_io1_n_d = '1' and cart_io1_n_i = '0' and ram_we_i = '1' and ram_addr_i = X"DE00" then
            ...
            when STAGE1_ST =>
               ...
               if cart_io1_n_d = '1' and cart_io1_n_i = '0' and ram_we_i = '1' and ram_addr_i = X"DE0E" then
                  state_ef3 <= STAGE2_ST;
               end if;
